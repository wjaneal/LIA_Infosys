...
class NotesController extends \BaseController {
	...
	{
	$view=Auth::user()->type;
	$notes = DB::table('notes')->join('users','users.id','=','notes.staffuid')->select('notes.date','notes.note', 'notes.referencetype', 'notes.referenceid', 'users.id','users.ulname','users.ufname')->get(); 
	$noteslist = array();
...
var_dump($noteslist);
var_dump($view);
	#return View::make('notes.index')->with('noteslist', $noteslist)->with('notes', $notes)->with('view',$view); 
	}
	...
	{
	$view=Auth::user()->type;
	$students = DB::table('students')->select('id', 'slname','sfname')->get();
	$teachers = DB::table('teachers')->select('id', 'tlname','tfname')->get();
	$agents = DB::table('agents')->select('id', 'alname','afname')->get();
...
#Function to select individual students or agents to display notes pertaining to them
	...
	return View::make('notes.selectindividual')->with('agent_data', $agent_data)->with('student_data', $student_data)->with('view', $view);
	}
######################################################################
...
#######################################################################
	...
	$select_type = 'agent';
	if ($data['agent'] != 'A'){
		$agent_list = Note::where('referenceid','=', $data['agent'])->where('referencetype','=','agents')->get();
		$name_data = Agent::findOrFail($data['agent']);
		}
	if ($data['student'] != 'S') {
		$select_type = 'student';
		$student_list = Note::where('referenceid', '=', $data['student'])->where('referencetype','=','students')->get();
		$name_data = Student::findOrFail($data['student']);
		}
#var_dump($name_data);
	return View::make('notes.listindividual')->with('agent_data', $agent_list)->with('student_data', $student_list)->with('select_type', $select_type)->with('view',$view)->with('name_data',$name_data);
	}
}
